...
# valid 의 역할은 훈련 중 검증하겠다.
# 그렇다면 #1에서 나누는 게 필요하냐 이거야
# 그러면 그냥 #3에서 검증할 때 그냥 validation_split 으로 이만큼만 너가 써주세요 라고 입력하면 된다.
# 1에서랑 3에서랑 아무거나. / 아무래도 #3 단계에서 파라미터만 조절하면 검증할 수 있는 데이터를 조절하기 쉽다.
