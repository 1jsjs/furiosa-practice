...
plt.plot(hist.history['loss'], c='red', label = 'loss') # y값만 넣으면 시간 순으로 그려줌
#plot: 선 긋기 / hist.history['loss'] 리스트 형태 데이터 / 순서가 없으면 1-10 순서대로 매칭이 된다. 명시하지 않으면 y값으로만 선을 긋는다.
plt.plot(hist.history['val_loss'], c='blue', label = 'val_loss') # y값만 넣으면 시간 순으로 그려줌
