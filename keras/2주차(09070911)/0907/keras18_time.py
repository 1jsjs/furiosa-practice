...
start_time = time.time() # 현재 시간 반환, 즉 시작시간
model.fit (x_train, y_train, epochs = 100, batch_size = 50) #실질적으로 시간이 제일 오래 걸리는 부분
end_time = time.time() # 현재 시간 반환, 즉 끝시간
#변수 start_time과 end_time 생성
...
print ("RMSE: ", rmse)
print ("걸린 시간 :", round(end_time - start_time,
                        2 #소수 둘째자리까지 올림
                        ), "초")
