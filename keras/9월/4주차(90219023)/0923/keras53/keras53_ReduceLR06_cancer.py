...
model.compile (loss = 'binary_crossentropy', optimizer =Adam(learning_rate=learning_rate),  #mse는 수치화된 것에 대한 것. / 이진 분류에서는 딱 하나만 쓴다. **binary_crossentropy**
            #    metrics=['accuracy'],
               metrics=['acc'],
               )
...
path_save= "./_save/keras53_ReduceLR06_cancer/"
...
model.fit (x_train, y_train,
                  ...
                  callbacks = [es,mcp,rlr], #EarlyStopping을 리스트로 받아드림. 
                  )
end_time = time.time()
